...
<?php } else { ?>
<script src="<?=base_url();?>assets/pages/js/form-validation-testimoni.js" type="text/javascript"></script>
<script>
var save_method;
var table;
...
$(document).ready(function() {
	...
	table = $('#tableData').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/testimoni/loadTable')?>",
            "type": "POST"
        },
...
function reset(){
	...
	$("#upload").hide();
    $('[name="id"]').val('');
    $('[name="testimoni_nama"]').val('');
    $('[name="testimoni_email"]').val('');
    $('[name="testimoni_pekerjaan"]').val('');
    $('[name="testimoni_testimoni"]').val('');
}
...
function showInputan(){
	...
	save_method = 'add';
	$("#inputan").show();
	$("input[name=testimoni_nama]").focus();
	$("#btnSave").text('Save');	
	$('#alerERRInput').hide();
}
...
function save(){ //change button text
	...
	if(save_method == 'add') {
		$('#btnSave').text('Saving...');
        url = "<?php echo site_url('backend/testimoni/save')?>";
		$("#msgSKS").text("Data Berhasil Ditambah !!");
		$("#msgERR").text("Data Gagal Ditambah !!");
    } else {
		$('#btnSave').text('Updating...');
        url = "<?php echo site_url('backend/testimoni/update')?>";
...
function edit(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('backend/testimoni/prepareEdit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			...
			save_method = 'update';
			$("#btnSave").text('Update');
            $('[name="id"]').val(data.testimoni_id);
            $('[name="testimoni_nama"]').val(data.testimoni_nama);
            $('[name="testimoni_email"]').val(data.testimoni_email);
            $('[name="testimoni_pekerjaan"]').val(data.testimoni_pekerjaan);
            $('[name="testimoni_testimoni"]').val(data.testimoni_testimoni);
            $('[name="testimoni_ind"]').val(data.testimoni_ind);
        },
        error: function (jqXHR, textStatus, errorThrown)
        {
...
$(document).on("click","#hapusData",function(){
	...
	swal({
		...
		function(){
		 $.ajax({
			url:"<?php echo site_url('backend/testimoni/delete');?>"+'/'+id,
			data: id,
			success: function(data){
				$('#tableData').DataTable().ajax.reload();
                  ...
                  <table id="tableData" class="table table-bordered table-striped">
                    <thead>
                      <tr>
						  ...
						  <th style="width: 15%">Testimoni</th>
						  <th style="width: 15%">Tampilkan</th>
              ...
              <div class="row">
				<div class="col-md-12">
					<div class="form-group">
						<label>Nama *</label>
						<input type="text" class="form-control" name="testimoni_nama" placeholder="Nama" maxlength="50" >
					</div>
				</div>
				<div class="col-md-12">
					<div class="form-group">
						<label>Email *</label>
						<input type="text" class="form-control" name="testimoni_email" placeholder="Email" maxlength="100">
					</div>
				</div>
				<div class="col-md-12">
					<div class="form-group">
						<label>Pekerjaan *</label>
						<input type="text" class="form-control" name="testimoni_pekerjaan" placeholder="Pekerjaan" maxlength="100">
					</div>
				</div>
				<div class="col-md-12">
					<div class="form-group">
						<label>Testimoni *</label>
						<textarea class="form-control" name="testimoni_testimoni" placeholder="Testimoni"></textarea>
					</div>
				</div>
				<div class="col-md-12">
					<div class="form-group">
						<label>Tampilkan *</label>
						<select class="form-control" name="testimoni_ind">
