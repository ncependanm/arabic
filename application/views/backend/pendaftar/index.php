...
$(document).ready(function() {
	...
	table = $('#tableData').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/pendaftar/loadTable')?>",
            "type": "POST"
        },
...
function reset(){
	...
	$("#upload").hide();
    $('[name="id"]').val('');
    $('[name="pendaftar_nama"]').val('');
    $('[name="pendaftar_email"]').val('');
    $('[name="pendaftar_no_telp"]').val('');
    $('[name="pendaftar_alamat"]').val('');
}
...
function showInputan(){
	...
	save_method = 'add';
	$("#inputan").show();
	$("input[name=pendaftar_nama]").focus();
	$("#btnSave").text('Save');	
	$('#alerERRInput').hide();
}
...
function save(){ //change button text
	...
	if(save_method == 'add') {
		$('#btnSave').text('Saving...');
        url = "<?php echo site_url('backend/pendaftar/save')?>";
		$("#msgSKS").text("Data Berhasil Ditambah !!");
		$("#msgERR").text("Data Gagal Ditambah !!");
    } else {
		$('#btnSave').text('Updating...');
        url = "<?php echo site_url('backend/pendaftar/update')?>";
...
function edit(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('backend/pendaftar/prepareEdit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			...
			save_method = 'update';
			$("#btnSave").text('Update');
            $('[name="id"]').val(data.pendaftar_id);
            $('[name="pendaftar_nama"]').val(data.pendaftar_nama);
            $('[name="pendaftar_email"]').val(data.pendaftar_email);
            $('[name="pendaftar_no_telp"]').val(data.pendaftar_no_telp);
            $('[name="pendaftar_alamat"]').val(data.pendaftar_alamat);
        },
        error: function (jqXHR, textStatus, errorThrown)
        {
...
$(document).on("click","#hapusData",function(){
	...
	swal({
		...
		function(){
		 $.ajax({
			url:"<?php echo site_url('backend/pendaftar/delete');?>"+'/'+id,
			data: id,
			success: function(data){
				$('#tableData').DataTable().ajax.reload();
              ...
              <div class="row ">
				<div class="col-md-12">
					<div class="form-group">
						...
						<input type="text" class="form-control" name="pendaftar_nama" placeholder="Nama Pendaftar" maxlength="50" >
					</div>
				</div>
				<div class="col-md-12">
					<div class="form-group">
						<label>Email Pendaftar *</label>
						<input type="text" class="form-control" name="pendaftar_email" placeholder="Email Pendaftar" maxlength="100">
					</div>
				</div>
				<div class="col-md-12">
					<div class="form-group">
						<label>No Telpon</label>
						<input type="text" class="form-control" name="pendaftar_no_telp" placeholder="No Telpon Pendaftar" maxlength="100">
					</div>
				</div>
				<div class="col-md-12">
					<div class="form-group">
						<label>Alamat</label>
						<textarea class="form-control" name="pendaftar_alamat" placeholder="Alamat Pendaftar"></textarea>
					</div>
				</div>
			  </div>
			</div>
