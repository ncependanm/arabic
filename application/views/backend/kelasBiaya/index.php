...
<?php } else { ?>
<script src="<?=base_url();?>assets/pages/js/form-validation-kelas-biaya.js" type="text/javascript"></script>
<script>
var save_method;
var table;
...
$(document).ready(function() {
	...
	table = $('#tableData').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/kelasBiaya/loadTable')?>",
            "type": "POST"
        },
...
function reset(){
	...
	$("#upload").hide();
    $('[name="id"]').val('');
    $('[name="kelasbiaya_nama"]').val('');
    $('[name="kelasbiaya_ket"]').val('');
    $('[name="kelasbiaya_biaya"]').val('');
}
...
function showInputan(){
	...
	save_method = 'add';
	$("#inputan").show();
	$("input[name=kelasbiaya_nama]").focus();
	$("#btnSave").text('Save');	
	$('#alerERRInput').hide();
}
...
function save(){ //change button text
	...
	if(save_method == 'add') {
		$('#btnSave').text('Saving...');
        url = "<?php echo site_url('backend/kelasBiaya/save')?>";
		$("#msgSKS").text("Data Berhasil Ditambah !!");
		$("#msgERR").text("Data Gagal Ditambah !!");
    } else {
		$('#btnSave').text('Updating...');
        url = "<?php echo site_url('backend/kelasBiaya/update')?>";
...
function edit(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('backend/kelasBiaya/prepareEdit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			...
			save_method = 'update';
			$("#btnSave").text('Update');
            $('[name="id"]').val(data.kelasbiaya_id);
            $('[name="kelasbiaya_nama"]').val(data.kelasbiaya_nama);
            $('[name="kelasbiaya_ket"]').val(data.kelasbiaya_ket);
            $('[name="kelasbiaya_biaya"]').val(data.kelasbiaya_biaya);
        },
        error: function (jqXHR, textStatus, errorThrown)
        {
...
$(document).on("click","#hapusData",function(){
	...
	swal({
		...
		function(){
		 $.ajax({
			url:"<?php echo site_url('backend/kelasBiaya/delete');?>"+'/'+id,
			data: id,
			success: function(data){
				$('#tableData').DataTable().ajax.reload();
              ...
              <div class="row">
				<div class="col-md-12">
					<div class="form-group">
						...
						<input type="text" class="form-control" name="kelasbiaya_nama" placeholder="Nama Kelas" maxlength="50" >
					</div>
				</div>
				<div class="col-md-12">
					<div class="form-group">
						<label>Keterangan Kelas *</label>
						<textarea class="form-control" name="kelasbiaya_ket"></textarea>
					</div>
				</div>
				<div class="col-md-12">
					<div class="form-group">
						<label>Harga Kelas *</label>
						<input type="text" class="form-control" name="kelasbiaya_biaya" placeholder="Harga Kelas" maxlength="20" >
					</div>
				</div>
			  </div>
			</div>
